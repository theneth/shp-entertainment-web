      ...
      <main className="flex-grow">
        ...
        <section id="experiences" className="w-full h-[100svh] bg-white text-black pt-20 pb-8 md:pt-24 md:pb-10 flex flex-col items-center overflow-hidden relative">
          ...
          <div className="flex-1 w-full flex items-center justify-center relative overflow-hidden">
            ...
            <div className="absolute left-0 top-0 bottom-0 w-12 md:w-48 bg-gradient-to-r from-white to-transparent z-20 pointer-events-none"></div>
            <div className="absolute right-0 top-0 bottom-0 w-12 md:w-48 bg-gradient-to-l from-white to-transparent z-20 pointer-events-none"></div>

            {/* Marquee Track */}
            <div className="animate-marquee hover:[animation-play-state:paused] gap-4 md:gap-8 px-2 md:px-4">
              {[...services, ...services].map((service, index) => (
                <div 
                  key={index} 
                  className="group relative overflow-hidden flex flex-col items-center justify-center w-[220px] md:w-[350px] lg:w-[400px] h-[160px] md:h-[260px] lg:h-[300px] flex-shrink-0 bg-white hover:bg-black hover:text-white rounded-2xl md:rounded-3xl transition-all duration-500 cursor-pointer shadow-[0_4px_20px_rgba(0,0,0,0.03)] hover:shadow-2xl border border-gray-100"
                >
                  {/* Subtle background glow on hover */}
                  <div className="absolute inset-0 bg-gradient-to-br from-white/10 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-500"></div>
                  
                  <service.icon className="w-8 h-8 md:w-14 md:h-14 mb-3 md:mb-6 stroke-[1.5] text-black group-hover:text-white group-hover:scale-110 group-hover:-translate-y-2 transition-all duration-500 relative z-10" />
                  
                  <h3 className="text-xs md:text-sm font-bold uppercase tracking-widest text-center relative z-10 group-hover:-translate-y-2 transition-all duration-500">
                    {service.name}
                  </h3>
